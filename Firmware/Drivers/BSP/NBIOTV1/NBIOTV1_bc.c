...
#ifndef NBIOTV1_VDDA_MV
#define NBIOTV1_VDDA_MV            (3300UL)
#endif
...
static void BC_Chg_Freq2Status(void);
static void BC_Chg_TIM_CaptureCallback(TIM_HandleTypeDef *htim);
static int32_t BC_Chg_TIM_Init(void);

static void BC_Sw_IO_Init(void);
static void BC_Sw_Cmd_Handler(void);
static void BC_Sw_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim);
static int32_t BC_Sw_TIM_Init(void);

static void BC_BatMs_ADC_MspInit(ADC_HandleTypeDef* adcHandle);
static void BC_BatMs_ADC_MspDeInit(ADC_HandleTypeDef* adcHandle);
static int32_t BC_BatMs_ADC_Channel_Init(void);

static void BC_Chg_TIM_MspInit(TIM_HandleTypeDef *htim);
static void BC_Chg_TIM_MspDeInit(TIM_HandleTypeDef *htim);

static int32_t BC_BatMs_ADC_Initialization(void);
static int32_t BC_BatMs_ADC_DeInitialization(void);
...
void BSP_BC_ChgPinHasToggled(TIM_HandleTypeDef *htim)
{
  ...
  else if(uhCaptureIndex == 1U)
  {
    ...
    /* Frequency computation: for this example TIMx (TIM15) is clocked by APB1Clk */
...
int32_t BSP_BC_GetVoltage(uint32_t *BatteryVoltage)
{
  ...
  // [0-3.3V]
  adc_voltage = __LL_ADC_CALC_DATA_TO_VOLTAGE(ADC4, NBIOTV1_VDDA_MV, adc_value, LL_ADC_RESOLUTION_12B);

  // [0-4.2V]

  float tempFloatBatteryVoltage = ((NBIOTV1_BC_ADC_VOLTAGE_DIVIDER_R1 + NBIOTV1_BC_ADC_VOLTAGE_DIVIDER_R2) * adc_voltage) / NBIOTV1_BC_ADC_VOLTAGE_DIVIDER_R2;

  *BatteryVoltage = (uint32_t) tempFloatBatteryVoltage;
...
int32_t BSP_BC_GetVoltageAndLevel(uint32_t *BatteryVoltage, uint32_t *BatteryLevel)
{
  ...
  static int32_t WindowPosition = 0;
  ...
  WindowVoltage[WindowPosition] = Voltage;
  WindowPosition++;

  /* Control if we have reached the end */
  if(WindowPosition == WINDOW_VOLTAGE_DIM)
  {
...
int32_t BSP_BC_ChargerEnableInit(void)
{
  ...
  HAL_GPIO_WritePin(STBC02_CE_GPIO_PORT, STBC02_CE_PIN, GPIO_PIN_SET);

  /* Enable GPIO channels Clock */
  STBC02_CE_GPIO_CLK_ENABLE();

  GPIO_InitStruct.Pin = STBC02_CE_PIN;
  GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_OD;
  GPIO_InitStruct.Pull = GPIO_PULLUP;
  GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_VERY_HIGH;

  HAL_GPIO_Init(STBC02_CE_GPIO_PORT, &GPIO_InitStruct);
...
{
  STBC02_CE_GPIO_CLK_DISABLE();

  HAL_GPIO_DeInit(STBC02_CE_GPIO_PORT, STBC02_CE_PIN);
...
{
  HAL_GPIO_WritePin(STBC02_CE_GPIO_PORT, STBC02_CE_PIN, GPIO_PIN_RESET);

  STBC02_Delay(STBC02_CE_REENABLE_DURATION);
...
/**
 * @brief  This method initializes the ADC peripheral used for Battery Voltage Conversion
 * @param  ADC_InitFor who wants to Init the ADC: Battery Charger
 * @retval BSP_ERROR_NONE in case of success
...
static int32_t BC_BatMs_ADC_Initialization(void)
{
  ...
  ADC4_Handle.Init.ScanConvMode = ADC_SCAN_DIRECTION_FORWARD;
  ...
  ADC4_Handle.Init.OversamplingMode = DISABLE;
...
/**
 * @brief  This method De initializes the ADC peripheral used for Battery Voltage Conversion
...
static int32_t BC_Chg_TIM_Init(void)
{
  ...
  /* TIM15 configuration: Input Capture mode ---------------------
   The external signal is connected to TIM15 CH1 (PB14)
  ...
  /* Set TIMx instance */
  BC_Chg_TIM_Handle.Instance = TIM15;
