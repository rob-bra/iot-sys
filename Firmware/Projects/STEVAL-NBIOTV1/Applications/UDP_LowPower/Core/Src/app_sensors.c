...
const char *eventNames[EVENT_TYPE_COUNT] =
{
    ...
    "MLC1 ",
    "FSM  ",
    "FORCE",  // FORCE_MEASUREMENT - event generated by user through web dashboard (pending command)
    };
...
void SensorsDataTask(void *pvParameters)
{
  ...
  for(;;)
  {
    ...
    /* TEST: print event type if is FORCED_MEASUREMENT */
    if(event == eEventForceMeasurement)
    {
      printf("\r\nForce measurement event received!\r\n");
    }

    /********************************************************************************
     * PULIZIA DEI TIPI DI EVENTO DURANTE LA FASE DI BASELINE HTTP
     *
     * Per evitare di processare eventi diversi da eEventFirst/eEventTimer/eEventForceMeasurement
     * durante la fase di baseline HTTP, ignoro gli altri eventi
     * */
    //
    if((event != eEventFirst) && (event != eEventTimer) && (event != eEventForceMeasurement))
    {
